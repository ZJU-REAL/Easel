...
Codex / Claude Code" 之类自曝措辞），就会**公开泄露且删不掉**。
...
- WARN 级 = AI 措辞（由 AI 生成 / Codex / Claude / Anthropic / system prompt / 大模型）与
...
SECRET_PATTERNS: list[tuple[str, str, "re.Pattern[str]", str]] = [
    ...
    ("internal-path", "med", _p(r"(?:~|/root)?/\.codex\S*"), "Codex 内部路径"),
    ("internal-path", "med", _p(r"(?:~|/root)?/\.easel-browser-profiles\S*"), "登录态 profile 路径"),
    ("internal-path", "med", _p(r"\bruntime[\\/]browser-profiles[\\/]\S*", re.I),
     "Seal 登录态 profile 路径"),
    ...
    ("ai-disclosure", "low", _p(r"\bCodex\b", re.I), "内部编排框架名"),
...
def cmd_selftest(_a) -> int:
    ...
    block_positives = [
        ...
        ("path profile", "登录态在 runtime/browser-profiles/XhsProfile"),
