...
def cmd_selftest(_a) -> int:
    ...
    expected_root = Path(os.environ.get("EASEL_BROWSER_PROFILES") or PROJECT_ROOT / "browser-profiles")
    assert _profile_dir(None).parent == expected_root.expanduser()
    custom_root = Path(tempfile.gettempdir()) / "xhs-profile-test"
    assert _profile_dir(str(custom_root)).parent == custom_root
    # 代理逻辑
