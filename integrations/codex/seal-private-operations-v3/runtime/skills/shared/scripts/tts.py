...
依赖：
    ...
    - ffmpeg / ffprobe（可选）：--format wav/m4a 转码 + 联网自测校验时长。
...
用法示例：
    ...
    tts.py voices
    tts.py --selftest                 # 离线检查命令构造
    tts.py --selftest-live            # 真实联网合成
"""
...
def _edge_tts_command(
    ...
    *,
    ...
    """Build an argv-only edge-tts command so it can be verified offline."""
    cmd = [binp, "--voice", voice, "--write-media", str(media_out), "--file", text_file]
    if rate:
        cmd += [f"--rate={rate}"]
    if volume:
        cmd += [f"--volume={volume}"]
...
def _run_edge_tts(
    ...
    *,
    ...
    cmd = _edge_tts_command(
        binp, text_file, media_out, voice=voice, rate=rate, volume=volume,
        pitch=pitch, subtitle=subtitle, proxy=proxy,
    )
...
def cmd_selftest(_args=None) -> int:
    """Offline contract test; live service availability is tested separately."""
    binp = _edge_tts_bin()
    with tempfile.TemporaryDirectory() as d:
        root = Path(d)
        ...
        text_file.write_text("配音测试，你好", encoding="utf-8")
        cmd = _edge_tts_command(
            binp, str(text_file), output, voice=DEFAULT_VOICE, rate="-5%",
            volume="+10%", pitch="+2Hz", subtitle=subtitle,
            proxy="http://127.0.0.1:7890",
        )
        expected = {
            "--voice", DEFAULT_VOICE, "--write-media", str(output), "--file",
            str(text_file), "--rate=-5%", "--volume=+10%", "--pitch=+2Hz",
            "--write-subtitles", str(subtitle), "--proxy", "http://127.0.0.1:7890",
        }
        if not expected.issubset(set(cmd)) or cmd[0] != binp:
            _die("selftest 失败：edge-tts 命令参数构造不完整", code=1)
        if _read_text(argparse.Namespace(file=str(text_file), text=None)) != "配音测试，你好":
            _die("selftest 失败：文本文件读取结果不一致", code=1)
    print("[PASS] selftest 通过：edge-tts 命令、参数与文本读取契约正确")
    return 0


def cmd_live_selftest() -> int:
    _edge_tts_bin()
    if not _env_proxy():
        print(
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        ns = argparse.Namespace(
            ...
            subtitle=None, proxy=None, format="mp3", engine="edge",
        )
        cmd_speak(ns)
    ...
    print("[PASS] 联网自测通过：edge-tts 真实合成 mp3 成功")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--selftest", action="store_true",
                    help="离线检查 edge-tts 命令构造、参数和文本读取")
    ap.add_argument("--selftest-live", action="store_true",
                    help="真实合成一句中文并断言产出有效 mp3（需联网）")
