...
def _find_root() -> Path:
    """定位 V3 运行根：优先 EASEL_ROOT，否则向上查找 outputs。"""
...
BUNDLED_SKILLS = Path(os.environ.get("SEAL_BUNDLED_SKILLS", PROJECT_ROOT.parent / "bundled-skills"))
...
def _forward_publish_log(platform: str, title: str, url: str, ptype: str,
    ...
    candidates = [BUNDLED_SKILLS / "skill-publish-log" / "scripts" / "log.py"]
...
def _locate_lunar():
    for c in (BUNDLED_SKILLS / "skill-event-calendar" / "scripts" / "lunar.py",):
        if c.is_file():
            return c
    return None
