...
def _profile_dir(base: str | None) -> Path:
    root = Path(base).expanduser() if base else Path(
        os.environ.get("EASEL_BROWSER_PROFILES") or PROJECT_ROOT / "browser-profiles"
    ).expanduser()
    return root / PROFILE_NAME


def _sync_redbook_cookies(context) -> bool:
    """Persist the active Seal login for V3's read-only Redbook CLI."""
    cookies = {
        item["name"]: item["value"]
        for item in context.cookies()
        if "xiaohongshu.com" in str(item.get("domain", "")) and item.get("name") and item.get("value")
    }
    if not cookies.get("a1") or not cookies.get("web_session"):
        return False
    target = Path(
        os.environ.get("REDBOOK_COOKIE_FILE")
        or PROJECT_ROOT / "browser-profiles" / "redbook" / "cookies.json"
    ).expanduser()
    ...
    payload = {
        ...
        "cookies": dict(sorted(cookies.items())),
    }
...
def _proxy(explicit: str | None, disable: bool) -> str | None:
    """默认直连；显式 --proxy 或 EASEL_XHS_PROXY_MODE=auto/proxy 才读取代理。"""
    if disable:
        return None
    if explicit:
        return explicit
    if os.environ.get('EASEL_XHS_PROXY_MODE', 'direct').lower() not in {'auto', 'proxy'}:
        return None
...
def _has_login(page, attempts: int = 4) -> bool:
    """页面重定向可能销毁 JS context；只对瞬时导航错误做有界重试。"""
    for attempt in range(attempts):
        try:
            return page.locator(SELECTORS['login_ok']).count() > 0
        except Exception as exc:
            if not any(s in str(exc).lower() for s in ('execution context was destroyed', 'navigation')):
                raise
...
def _check_login_network(page) -> None:
    if 'website-login/error' in page.url or '安全限制' in (page.title() or ''):
        raise RuntimeError('小红书限制了当前网络访问（可能为 300012 风险 IP）。请关闭系统 VPN/全局代理，使用正常国内网络后重试。')
...
def cmd_login(a) -> int:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            login_state.write_status(sf, 'error', str(exc)[:500])
...
def cmd_whoami(a) -> int:
    ...
    try:
        with sync_playwright() as p:
            ctx = _launch(p, headed=getattr(a, 'headed', False), base=a.profile_base, proxy=_proxy(a.proxy, a.no_proxy))
            page = ctx.pages[0] if ctx.pages else ctx.new_page()
            try:
                page.goto(EXPLORE_URL, wait_until="domcontentloaded")
                try:  # 等任一明确状态；二维码加载可能晚于页面 DOM 就绪。
                    page.wait_for_selector(f"{SELECTORS['login_ok']}, {SELECTORS['qrcode']}", timeout=15000)
                except Exception:
                    pass
                _check_login_network(page)
                logged = _has_login(page)
                if not logged and not page.locator(SELECTORS['qrcode']).count():
                    raise RuntimeError('未检测到账号或登录二维码，无法确认登录状态')
...
def main() -> int:
    ...
    def add_common(p):
        p.add_argument("--profile-base", help="登录态根目录（默认 V3 browser-profiles）")
        p.add_argument("--proxy", help="显式指定浏览器代理；默认直连国内网络")
    ...
    add_common(p)
    p.add_argument('--headed', action='store_true', help='打开本地浏览器校验，供桌面工作台使用')
