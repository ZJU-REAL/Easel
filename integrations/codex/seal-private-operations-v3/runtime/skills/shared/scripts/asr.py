...
_MODELSCOPE_API = "https://modelscope.cn/api/v1/models/Systran/faster-whisper-{model}/repo/files"
_MODELSCOPE_FILE = "https://modelscope.cn/models/Systran/faster-whisper-{model}/resolve/master/{path}"
...
def _download_modelscope_model(model: str, destination: Path) -> Path:
    """Download a verified Systran model from ModelScope when Hugging Face is unreachable."""
    try:
        import requests
    except ImportError:
        _die("缺少 requests，无法使用 ModelScope 模型镜像。", 3)
    destination.parent.mkdir(parents=True, exist_ok=True)
    print(f"[asr] Hugging Face 不可用，切换 ModelScope 镜像下载 {model}（无需 Key）……", file=sys.stderr)
    try:
        response = requests.get(
            _MODELSCOPE_API.format(model=quote(model, safe="")),
            params={"Revision": "master", "Root": ""},
            timeout=(15, 60),
        )
        response.raise_for_status()
        payload = response.json()
        files = {item.get("Name"): item for item in payload["Data"]["Files"]}
    except Exception as exc:  # noqa: BLE001
        _die(f"读取 ModelScope 模型清单失败：{exc}", 4)
    missing = [name for name in _MODEL_FILES if name not in files or not files[name].get("Sha256")]
    if missing:
        _die(f"ModelScope 模型清单缺少必要文件或 SHA-256：{missing}", 4)
    ...
    try:
        for name in _MODEL_FILES:
            ...
            print(f"[asr] 下载 {name}（{expected_size / 1024 / 1024:.1f} MB）……", file=sys.stderr)
            with requests.get(
                _MODELSCOPE_FILE.format(model=quote(model, safe=""), path=quote(name)),
                stream=True,
                timeout=(15, 300),
            ) as download:
                ...
                with output.open("wb") as handle:
                    for chunk in download.iter_content(1024 * 1024):
                        ...
                        digest.update(chunk)
                        size += len(chunk)
            if (expected_size and size != expected_size) or digest.hexdigest().lower() != expected_sha:
                _die(f"ModelScope 文件校验失败：{name}", 4)
        ...
        raise
    print(f"[asr] 模型已校验并保存：{destination}", file=sys.stderr)
    return destination
...
def _render_json(cues: list[tuple[float, float, str]], meta: dict) -> str:
    ...
    segments = meta.get("_word_segments") or [
...
def _transcribe_audio(audio: Path, a) -> tuple[list[tuple[float, float, str]], dict]:
    ...
    if "/" not in a.model and not Path(a.model).exists():
        model_cache = Path(
            os.environ.get("EASEL_MODEL_CACHE")
            or Path(os.environ.get("EASEL_ROOT", Path.cwd())) / "outputs" / "_models"
        ).expanduser()
        local = model_cache / f"faster-whisper-{a.model}"
    ...
    try:
        ...
        if local is not None and model_ref == a.model and a.model in _MODELS:
            local = _download_modelscope_model(a.model, local)
            try:
                model = WhisperModel(str(local), device=a.device, compute_type=a.compute_type)
            except Exception as fallback_error:  # noqa: BLE001
                _die(f"ModelScope 回退模型加载失败：{fallback_error}", 4)
        else:
            hint = f"\n提示：可手动下载模型到 {local}/" if local is not None else ""
            _die(f"加载模型失败（可能是模型下载失败/网络/代理问题）: {e}{hint}", 4)
    ...
    segments, tinfo = model.transcribe(
        ...
        word_timestamps=bool(getattr(a, "word_timestamps", False)),
    )
    ...
    for seg in segments:
        ...
        if not text:
            continue
        if getattr(a, "word_timestamps", False):
            words = [
                {"start": round(float(word.start), 3), "end": round(float(word.end), 3), "word": word.word}
                for word in (seg.words or [])
                if word.start is not None and word.end is not None and (word.word or "").strip()
            ]
    ...
    if getattr(a, "word_timestamps", False):
        meta["_word_segments"] = word_segments
    return cues, meta
...
# ── selftest ─────────────────────────────────────────────────────────
def _selftest() -> int:
    """用随 V3 分发的中文语音夹具转写 SRT，避免把在线 TTS 故障误报为 ASR 故障。"""
    if not _has_ffmpeg():
        print("[WARN] 未找到 ffmpeg，仅在纯音频路径下自检", file=sys.stderr)

    with tempfile.TemporaryDirectory() as d:
        runtime_root = Path(os.environ.get("EASEL_ROOT", Path(__file__).resolve().parents[3]))
        fixture = runtime_root / "tests" / "assets" / "asr-test.wav"
        if not fixture.is_file() or fixture.stat().st_size == 0:
            print(f"[FAIL] 缺少 ASR 离线测试音频：{fixture}", file=sys.stderr)
            return 1

        srt = Path(d) / "asr_test.srt"
        ns = argparse.Namespace(
            input=str(fixture), output=str(srt), format="srt",
            model="tiny", language="zh", device="cpu", compute_type="int8",
...
def _add_transcribe_args(p) -> None:
    ...
    p.add_argument("--word-timestamps", action="store_true",
                   help="JSON 输出中保留逐词时间戳，供动态字幕等能力使用")
