...
用法：
    # 统计字数（从文件）
    python wordcount.py count -f draft.txt
    # 统计字数（从 stdin）
    echo "文本内容" | python wordcount.py count
    # 校验是否命中目标字数（默认 ±5% 容差）
    python wordcount.py check --target 140 -f draft.txt
    python wordcount.py check --target 140 --tolerance 0.1 < draft.txt
