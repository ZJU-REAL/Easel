...
由 Codex 按 AGENTS.md 自行读取 `profiles/<X>/` 并凝练，每个请求自包含，无全局文件污染。
...
def persona_prefix(name: str | None) -> str:
    ...
    明确账号记忆作用域，避免全局 MEMORY.md 污染并行画像会话。
    """
...
def chat_turn_message(user_message: str, name: str | None) -> str:
    """构造发给 Codex 的一轮对话消息：画像前缀（如有）+ 用户原文 + 末尾行为提醒。

    末尾提醒对抗长对话里「忘记先查 SKILL」的指令衰减（见 TURN_REMINDER）。
    对用户不可见（前端只显示用户原文），只进 Codex 上下文。
    """
