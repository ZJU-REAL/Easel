...
BUNDLED_SKILLS = PROJECT_ROOT.parent / "bundled-skills"
sys.path.insert(0, str(BUNDLED_SKILLS / "paper-explainer" / "scripts"))
...
def test_find_skill_prefixed_name_resolution():
...
def test_persona_skill_prioritizes_positioning_and_caps_cross_niche_scores():
    skill = (BUNDLED_SKILLS / "skill-persona-check/CAPABILITY.md").read_text()
...
def test_chat_stop_waits_for_codex_supervisor_cleanup():
    import threading

    async def scenario():
        session_id = "stop-cleanup-test"
        cancel = threading.Event()
        web._RUNNING_CHAT[session_id] = cancel
    ...
    assert result == {"stopped": True, "requested": True}
    assert cancel.is_set()


def test_codex_job_events_are_isolated(tmp_path, monkeypatch):
    monkeypatch.setattr(web, "SESSIONS_DIR", tmp_path)
    first = web._job_event_file("a")
    second = web._job_event_file("b")
    first.parent.mkdir(parents=True)
    first.write_text(json.dumps({"id": 1, "event": "token", "data": "own"}) + "\n")
    second.write_text(json.dumps({"id": 1, "event": "token", "data": "foreign"}) + "\n")
    assert web._read_job_events("a")[0]["data"] == "own"
    assert web._read_job_events("b")[0]["data"] == "foreign"
