...
export function verifyStale(platforms: string[], opts: VerifyOpts = {}): void {
  ...
  // 小红书使用可见浏览器，不在打开页面时自动弹窗；扫码成功或手动校验时再查。
  const eligible = opts.force ? platforms : platforms.filter((p) => p !== 'xiaohongshu');
  const todo = opts.force ? eligible : eligible.filter((p) => !isFresh(cache[p], ttlMs));
