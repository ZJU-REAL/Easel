...
export interface ChatSession {
  ...
  persona?: string;
  created: number;
  sessionKey?: string;  // Codex 会话映射键，用于后端删除
...
// One-time reset after changing the server-side Codex session store.
...
export function createSession(persona?: string, id?: string): ChatSession {
  return {
    id: id || generateId(),
    title: '新对话',
    messages: [],
...
export function updateSessionTitle(session: ChatSession): void {
  if (session.messages.length > 0 && ['New Chat', '新对话'].includes(session.title)) {
