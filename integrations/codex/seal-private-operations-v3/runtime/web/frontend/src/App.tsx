...
import { fetchStatus, fetchPersonas, streamChat, fetchLastTurn, stopChat, fetchOpsTasks } from './lib/api';
import type { PersonaItem, UploadedFile } from './lib/api';
import { deleteSession as deleteRemoteSession, bindOpsTaskConversation, deleteOpsTask } from './lib/api';
import {
  loadSessions,
  saveSessions,
  createSession,
...
export default function App() {
  ...
  const [activeTaskId, setActiveTaskId] = useState<string | null>(() => sessionStorage.getItem('seal_v3_active_task'));
  const [dataScopeTaskId, setDataScopeTaskId] = useState<string | null>(() => sessionStorage.getItem('seal_v3_data_scope_task'));
  const [, setActiveTaskTitle] = useState('');
  const [activeTask, setActiveTask] = useState<import('./lib/api').OpsTask | null>(null);
  const [opsTasksForPages, setOpsTasksForPages] = useState<import('./lib/api').OpsTask[]>([]);
  const navigateWithTask = useCallback((page: Page, taskId?: string) => {
    if (taskId) {
      setActiveTaskId(taskId);
      setDataScopeTaskId(taskId);
      try { sessionStorage.setItem('seal_v3_active_task', taskId); } catch { /* ignore */ }
      try { sessionStorage.setItem('seal_v3_data_scope_task', taskId); } catch { /* ignore */ }
      fetchOpsTasks().then((ts) => {
        const task = ts.find((t) => t.task_id === taskId);
        setActiveTask(task || null); setActiveTaskTitle(task?.title || '');
        if (task?.conversation_id) {
          const cid = task.conversation_id;
          setSessions((prev) => {
            if (prev.some((s) => s.id === cid)) return prev;
            const ns = createSession(undefined, cid);
            ns.taskId = task.task_id; ns.taskTitle = task.title; ns.title = `运营任务 · ${task.title}`;
            const next = [ns, ...prev]; saveSessions(next); return next;
          });
  ...
  //    则开一个新会话，避免两个窗口撞同一会话 → Codex 会话并发 崩溃（后端还有 flock 兜底）。
  ...
  // Restore the task banner and task-scoped conversation after a hard refresh.
  useEffect(() => {
    if (!activeTaskId) return;
    fetchOpsTasks().then((ts) => {
      const task = ts.find((t) => t.task_id === activeTaskId) || null;
      setActiveTask(task);
  ...
  useEffect(() => { fetchOpsTasks().then(setOpsTasksForPages).catch(() => {}); }, [activeTaskId, currentPage]);
  ...
  // Fetch status on mount — 真实反映 Codex 运行状态 + 首次引导检测
  useEffect(() => {
    fetchStatus()
      .then((data) => {
        setPersonas(data.personas || []);
        setRuntimeStatus(data.runtime ? 'connected' : 'disconnected');
  ...
  const handleTaskCreated = useCallback(async (task: import('./lib/api').OpsTask) => {
    setOpsTasksForPages((prev) => [task, ...prev.filter((x) => x.task_id !== task.task_id)]);
    const ns = createSession(selectedPersona || undefined, task.conversation_id || undefined);
    ns.taskId = task.task_id; ns.taskTitle = task.title;
    ns.title = `运营任务 · ${task.title}`;
    const updated = [ns, ...sessionsRef.current];
    saveSessions(updated); setSessions(updated); setActiveSessionId(ns.id);
    try { await bindOpsTaskConversation(task.task_id, ns.id); } catch { /* task remains usable locally */ }
    navigateWithTask('chat', task.task_id);
  }, [selectedPersona, navigateWithTask]);

  const handleTaskDeleted = useCallback((task: import('./lib/api').OpsTask) => {
    setOpsTasksForPages((prev) => prev.filter((x) => x.task_id !== task.task_id));
    const id = task.conversation_id;
    if (!id) return;
    setSessions((prev) => {
      const next = prev.filter((s) => s.id !== id && s.taskId !== task.task_id);
      saveSessions(next);
      return next;
    });
    if (activeSessionId === id || activeTaskId === task.task_id) {
      setActiveTaskId(null); setActiveTask(null); setActiveTaskTitle('');
      try { sessionStorage.removeItem('seal_v3_active_task'); } catch { /* ignore */ }
  ...
  const startStream = useCallback((
    ...
    streamCtl.current[sessionId] = streamChat(
      ...
      false,
      undefined,
      attachments, sessionsRef.current.find((s) => s.id === sessionId)?.taskId,
    );
  ...
  const handleNewChat = useCallback(() => {
    setActiveTaskId(null); setActiveTask(null); setActiveTaskTitle(''); setDataScopeTaskId(null);
    try { sessionStorage.removeItem('seal_v3_active_task'); } catch { /* ignore */ }
    try { sessionStorage.removeItem('seal_v3_data_scope_task'); } catch { /* ignore */ }
  ...
  const handleSessionSelect = useCallback((id: string) => {
    ...
    if (target) {
      ...
      setActiveTaskId(target.taskId || null);
      setDataScopeTaskId(target.taskId || null);
      if (!target.taskId) { setActiveTask(null); setActiveTaskTitle(''); }
      try {
        if (target.taskId) {
          sessionStorage.setItem('seal_v3_active_task', target.taskId);
          sessionStorage.setItem('seal_v3_data_scope_task', target.taskId);
        } else {
          sessionStorage.removeItem('seal_v3_active_task');
          sessionStorage.removeItem('seal_v3_data_scope_task');
        }
      } catch { /* ignore */ }
    }
    setCurrentPage('chat');
  ...
  const exitTask = useCallback(() => {
    const normal = sessionsRef.current.find((s) => !s.taskId);
    setActiveTaskId(null); setActiveTask(null); setActiveTaskTitle(''); setDataScopeTaskId(null);
    try { sessionStorage.removeItem('seal_v3_active_task'); sessionStorage.removeItem('seal_v3_data_scope_task'); } catch { /* ignore */ }
    if (normal) handleSessionSelect(normal.id); else handleNewChat();
  }, [handleNewChat, handleSessionSelect]);

  const handleSessionDelete = useCallback((id: string) => {
    if (!window.confirm('确定删除这条对话？')) return;

    const target = sessionsRef.current.find((s) => s.id === id);
    // A task conversation is a first-class work unit. Deleting it from the
    // sidebar must also delete the task entity, while ordinary chats remain
    // independent and are only removed locally/Codex-map-wise.
    if (target?.taskId) {
      void deleteOpsTask(target.taskId).catch((err) => {
        alert(err instanceof Error ? err.message : '删除运营任务失败');
      });
    }
    ...
    if (target?.sessionKey) {
      // Do not delete the Codex session record while its agent is still
    ...
    if (target?.taskId && (target.taskId === activeTaskId || id === activeSessionId)) {
      setActiveTaskId(null); setActiveTask(null); setActiveTaskTitle('');
      try { sessionStorage.removeItem('seal_v3_active_task'); } catch { /* ignore */ }
    }
  }, [activeSessionId, activeTaskId, clearStream]);
  ...
  const renderPage = () => {
    switch (currentPage) {
      ...
      case 'chat':
        return activeSession ? (
          <ChatPage
            ...
            taskInfo={activeTask ? { taskId: activeTask.task_id, title: activeTask.title, platform: activeTask.platform, scenario: activeTask.scenario, stage: activeTask.stages?.[activeTask.current_stage_index]?.name || '', accountId: activeTask.account_id, goal: activeTask.goal, status: activeTask.status } : null}
      ...
      case 'ideas':
        return <IdeasPage onUseTopic={handleUseTopic} taskId={dataScopeTaskId} workTaskId={activeTaskId} tasks={opsTasksForPages} />;
      case 'calendar':
        return <CalendarPage taskId={dataScopeTaskId} workTaskId={activeTaskId} tasks={opsTasksForPages} />;
      case 'publish':
      ...
      case 'outputs':
        return <OutputsPage taskId={dataScopeTaskId} />;
      case 'accounts':
        return <AccountsPage />;
      case 'profile':
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        {(['trends', 'ideas', 'calendar', 'publish', 'breakdown'] as Page[]).includes(currentPage) && (
          <SubNav current={currentPage} onNavigate={(p) => navigateWithTask(p)} />
        )}
        <div className="page-host">
          {activeTask && currentPage !== 'dashboard' && (
            <TaskContextCard task={{ taskId: activeTask.task_id, title: activeTask.title, platform: activeTask.platform, scenario: activeTask.scenario, accountId: activeTask.account_id, goal: activeTask.goal, status: activeTask.status, stage: activeTask.stages?.[activeTask.current_stage_index]?.name || '' }} onExit={exitTask} />
          )}
          {renderPage()}
        </div>
      </main>
      ...
      {showRecommend && (
        <div className="overlay">
          <div className="modal" style={{ width: 420, maxWidth: '100%', textAlign: 'center' }}>
            ...
            <h2 style={{ margin: '12px 0 8px', fontSize: 20 }}>欢迎使用 Seal</h2>
