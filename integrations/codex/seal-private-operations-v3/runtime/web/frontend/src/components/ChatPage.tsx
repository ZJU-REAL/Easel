...
interface ChatPageProps {
  ...
  taskInfo?: { taskId: string; title: string; platform: string; scenario: string; stage: string; accountId?: string|null; goal?: string; status?: string } | null;
}
...
export default function ChatPage({ session, stream, onSend, onStop, onResend, taskInfo }: ChatPageProps) {
  ...
  const taskStatusLabel: Record<string, string> = { draft: '草稿', ready: '待开始', active: '进行中', paused: '已暂停', blocked: '已阻塞', completed: '已完成', cancelled: '已终止' };
  const taskBanner = taskInfo ? <div className="chat-task-banner"><span>当前任务</span><strong>{taskInfo.title}</strong><em>{taskInfo.platform} · {taskInfo.stage || '准备阶段'} · {taskStatusLabel[taskInfo.status || 'draft'] || '未知状态'}</em></div> : null;
