...
import {
  ...
  accountWhoami, logoutAccount, submitLoginSms, cancelLogin,
} from '../lib/api';
...
export default function AccountsPage() {
  ...
  const runWhoami = useCallback((platform: string) => {
    ...
    accountWhoami(platform)
      ...
      .catch((e) => {
        setErr(e instanceof Error ? e.message : '账号校验未完成，请检查网络后重试');
  ...
  const closeQr = useCallback(async () => {
    stopPoll();
    try {
      await cancelLogin(qrPlatformRef.current);
    } catch (e) {
      setErr(e instanceof Error ? e.message : '取消登录失败，请稍后重试');
      return;
    }
  ...
  const handleLogin = useCallback(async (a: AccountItem) => {
    ...
    setQrNonce((n) => n + 1);
    stopPoll();
    setQr({ platform: a.platform, name: a.name, state: 'starting', message: '正在准备二维码；若账号校验正在运行，请稍候。', qr: '' });
    try {
      const res = await startLogin(a.platform);
      if (!aliveRef.current || qrPlatformRef.current !== a.platform) return;
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (['success', 'expired', 'error', 'cancelled'].includes(s.state)) {
            stopPoll();
      ...
      setQr((prev) => prev && ({ ...prev, state: 'error', message: '启动失败，请关闭后重试。' }));
    } finally {
      setBusy('');
    }
  }, [stopPoll, runWhoami]);
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        <div>
          ...
          <p className="page-subtitle">
            使用对应手机 App 扫码并确认，登录态仅保存在本机。<br />
            小红书默认直连国内网络。如遇 300012 网络风险提示，请关闭系统 VPN 或全局代理后重新获取二维码。
            小红书登录与手动校验会打开本地浏览器窗口，扫码期间请保持窗口开启。
          </p>
        </div>
        ...
        {accounts.map((a) => {
          ...
          return (
            <div key={a.platform} className="card account-card" style={{ opacity: a.supported ? 1 : 0.6 }}>
              ...
              {!logged && (
                <div className="account-card-note">{a.note || '扫码连接账号，授权后用于查询与发布。'}</div>
              )}
          ...
          <div className="modal" style={{ width: 360, maxWidth: '100%', textAlign: 'center' }}
            ...
            {qr.state === 'sms_required' ? (
              ...
              <div style={{ fontSize: 13, color: 'var(--red)', padding: 30 }}>
                {qr.message || '登录失败'}<br />请检查网络后重新获取二维码。
              </div>
            ) : (
              <div className="loading" style={{ padding: 40 }}><div className="spinner" />准备二维码…</div>
            )}
            <div style={{ marginTop: 16 }}>
              {['error', 'expired', 'cancelled'].includes(qr.state) && <button className="btn btn-primary" style={{ marginRight: 8 }}
                disabled={!!busy} onClick={async () => {
                  try {
                    await cancelLogin(qr.platform);
                    const account = accounts.find((a) => a.platform === qr.platform);
                    if (account) await handleLogin(account);
