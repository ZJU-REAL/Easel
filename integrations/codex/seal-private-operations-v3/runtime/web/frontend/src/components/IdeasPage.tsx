...
export default function IdeasPage({ onUseTopic, taskId, workTaskId, tasks = [] }: IdeasPageProps) {
  const [ideas, setIdeas] = useState<Idea[]>([]);
  const [form, setForm] = useState<IdeaInput | null>(null);
  const [editId, setEditId] = useState<string | null>(null);
  const [toast, setToast] = useState('');

  const load = useCallback(() => { fetchIdeas(taskId || undefined).then(setIdeas).catch(() => {}); }, [taskId]);
  useEffect(() => { load(); }, [load]);
  ...
  const openNew = () => { setEditId(null); setForm({ ...EMPTY, task_id: workTaskId || undefined }); };
  const openEdit = (it: Idea) => { setEditId(it.id); setForm({ title: it.title, note: it.note, source: it.source, status: it.status, task_id: it.task_id }); };
  const save = async () => {
    if (!form || !form.title.trim()) return;
    if (editId) await updateIdea(editId, form); else await createIdea({...form, task_id: form.task_id || undefined});
  ...
  const schedule = async (it: Idea) => {
    const d = new Date();
    await createSchedule({ title: it.title, date: d.toISOString().slice(0, 10), platform: '', time: '', status: 'idea', note: it.note, task_id: it.task_id || undefined });
    showToast('已加入日历（今天）');
  };
            ...
            <div className="kanban-list">
              ...
              {byStatus[col.key].map((it) => (
                <div key={it.id} className="card idea-card">
                  ...
                  <span className="badge" style={{ marginTop: 6 }}>归属：{it.task_id ? (tasks.find((t) => t.task_id === it.task_id)?.title || '运营任务') : '全局'}</span>
      ...
      {form && (
        <div className="overlay" onClick={() => setForm(null)}>
          <div className="modal" style={{ width: 440, maxWidth: '100%' }} onClick={(e) => e.stopPropagation()}>
            ...
            <label className="field-label">归属</label>
            <select className="field" value={form.task_id || ''} onChange={(e) => setForm({ ...form, task_id: e.target.value || undefined })}>
              <option value="">全局</option>
              {tasks.map((t) => <option key={t.task_id} value={t.task_id}>任务：{t.title}</option>)}
            </select>
