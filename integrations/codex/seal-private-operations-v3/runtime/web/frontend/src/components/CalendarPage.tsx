...
export default function CalendarPage({ taskId, workTaskId, tasks = [] }: { taskId?: string | null; workTaskId?: string | null; tasks?: { task_id: string; title: string }[] } = {}) {
  ...
  const load = useCallback(() => {
    fetchSchedule(taskId || undefined).then(setItems).catch(() => {});
  ...
  const openNew = (date: string) => {
    setDayView(null);
    setEditing(null);
    setForm({ ...EMPTY, date, kind: filter === 'event' ? 'event' : 'content', task_id: workTaskId || undefined });
  };
  ...
  const renderChip = (it: ScheduleItem) => {
    if (kindOf(it) === 'event') {
      return (
        <div key={it.id} className="cal-event is-event"
          title={`${it.event_type ? `${it.event_type}·` : ''}${it.title} · 归属：${it.task_id ? (tasks.find((t) => t.task_id === it.task_id)?.title || '运营任务') : '全局'}`}
    ...
    return (
      <div key={it.id} className="cal-event" title={`${it.title} · 归属：${it.task_id ? (tasks.find((t) => t.task_id === it.task_id)?.title || '运营任务') : '全局'}`}
  ...
  const save = async () => {
    ...
    else await createSchedule({...form, task_id: form.task_id || undefined});
      close(); load();
