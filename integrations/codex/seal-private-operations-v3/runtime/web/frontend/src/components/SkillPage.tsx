...
export default function SkillPage({ persona }: SkillPageProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return skills.filter((s) =>
      s.name.toLowerCase().includes(q) || (s.displayName || '').toLowerCase().includes(q) || (s.description || '').toLowerCase().includes(q));
  }, [skills, query]);
              ...
              {grouped[layer.key].map((s) => {
                ...
                return (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setSelected(s.name); } }}
                  ...
                  >
                    ...
                    <div className="skill-card-name">{s.displayName || s.name}</div>
                    <div className="skill-card-slug">{s.name}</div>
                    <div className="skill-card-desc">{s.displayDescription?.trim() || s.description?.trim() || LAYER_DESC[layer.key]}</div>
