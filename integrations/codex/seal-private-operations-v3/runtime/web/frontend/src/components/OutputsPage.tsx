...
import { fetchOutputs, fetchOutputContent, mediaUrl, deleteOutput, createProject } from '../lib/api';
...
export default function OutputsPage({ taskId }: { taskId?: string | null } = {}) {
  ...
  const load = useCallback(() => {
    setTreeError('');
    fetchOutputs(taskId || undefined).then(setRoots).catch(() => setTreeError('加载产物列表失败'));
  }, [taskId]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => { setStack([]); setSelected(null); }, [taskId]);
  ...
  return (
    <div className="gallery-page">
      <div className="gallery-head">
        <div>
          ...
          <p className="page-subtitle">
            ...
            {taskId ? ' · 当前归属：任务内容' : ' · 当前归属：全局内容'}
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}><button className="btn btn-sm btn-primary" onClick={() => setShowCreate(true)}>＋ 新建内容项目</button><button className="btn btn-sm" onClick={load}><IconRefresh size={14} /> 刷新</button></div>
      </div>
      ...
      {showCreate && <div className="overlay" onClick={() => setShowCreate(false)}><div className="modal" onClick={(e) => e.stopPropagation()} style={{width:420}}><h3>新建内容项目</h3><p style={{fontSize:13,color:'var(--text-secondary)',margin:'8px 0 14px'}}>创建后会出现在当前内容库范围。</p><input className="field" autoFocus placeholder="项目名称" value={projectTitle} onChange={(e)=>setProjectTitle(e.target.value)} /><div style={{display:'flex',justifyContent:'flex-end',gap:8,marginTop:16}}><button className="btn btn-sm" onClick={()=>setShowCreate(false)}>取消</button><button className="btn btn-sm btn-primary" disabled={!projectTitle.trim()} onClick={async()=>{ await createProject(projectTitle.trim(), taskId || undefined); setProjectTitle(''); setShowCreate(false); load(); }}>创建</button></div></div></div>}
    </div>
  );
}
