...
3. **高级感视觉（走 card-design，别做糙卡）**：卡片视觉**统一走 [card-design](../card-design/CAPABILITY.md) 设计系统**（选风格锁 spec、字大字细、填满画幅、禁大 emoji 廉价感）。文案要口语化、真实感，但**视觉不能糙**——想要「素人 / 手账」质感就选 card-design 的**手账贴纸 / 奶油温柔**风格（仍是高质量 HTML 渲染，不是糙 t2i 大 emoji）。
...
```bash
python ../bundled-skills/xhs-note-creator/scripts/analyze_material.py <path>... \
...
| 策略 | 适用 | 渲染路径 |
|------|------|----------|
| `html_card` | 纯文字卡 / 概念卡 / 数据卡（**默认、最常用**） | **走 [card-xiaohongshu](../card-xiaohongshu/CAPABILITY.md)**：读 [card-design](../card-design/CAPABILITY.md) 选风格锁 spec → 写 HTML → `render_card.py` 出图 → `card_audit.py` 硬门禁 |
...
目录名走脚本标准化：`python ../bundled-skills/xhs-note-creator/scripts/normalize_slug.py "标题" --with-ts`
...
```bash
python ../bundled-skills/xhs-note-creator/scripts/validate_meta.py outputs/主题名/meta.json
```

**② 视觉门**（仅 `html_card` 路径，渲染后跑；死空白/密度硬门禁）：

```bash
python ../bundled-skills/card-design/scripts/card_audit.py audit -f "outputs/主题名/card_*.png"
```
...
- **卡片视觉** → [card-design](../card-design/CAPABILITY.md)（设计系统/风格库，出图前必读）+ [card-xiaohongshu](../card-xiaohongshu/CAPABILITY.md)（HTML→截图渲染管线）
