...
```bash
python ../bundled-skills/xhs-note-creator/scripts/analyze_material.py input.jpg \
...
**做法**：交给 [card-xiaohongshu](../../card-xiaohongshu/CAPABILITY.md)——读 [card-design](../../card-design/CAPABILITY.md) 选风格锁 spec → 写 HTML → `render_card.py` 渲染 → `card_audit.py` 硬门禁。**不再用糙 t2i / 大 emoji 素人卡**；数据卡/概念卡也走 card-design 的对应骨架，视觉质量有保证。
...
```bash
python ../bundled-skills/xhs-note-creator/scripts/text_on_image.py input.jpg output.jpg \
  --text "一句钩子" \
  --position top \
  --fit 3x4 \
  --size 72 \
...
```bash
python ../bundled-skills/xhs-note-creator/scripts/collage_3x4.py output.jpg \
