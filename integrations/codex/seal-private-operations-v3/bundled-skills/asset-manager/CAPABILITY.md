...
2. 统一入口（默认 `--root outputs`）：
   ```bash
   S=../bundled-skills/asset-manager/scripts/assets.py
   python $S scan                                    # 生成/更新 INDEX.json
   python $S search "关键词" --platform xiaohongshu --type card --date this-week
   python $S search --tag 护肤,种草                   # 标签需全部命中
   python $S list --platform weibo                    # 素材清单
   python $S report                                   # 类型/平台/日期分布
   python $S tag "招人贴/card_1.png" --add 护肤,小红书 --remove 草稿
   python $S archive                                  # 预览归档计划（dry-run）
   python $S archive --apply                          # 确认后真正移动
   python $S archive 招人贴 --apply                    # 只归档某目录/文件
   ```
