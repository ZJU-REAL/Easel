...
5. 自检:
   ```bash
   python ../bundled-skills/skill-wechat-publisher/scripts/multi_publish.py --check
   ```
   两项都打 `✓` 说明就绪。
...
```bash
python ../bundled-skills/skill-wechat-publisher/scripts/publish.py --account main --input x.md --cover x.jpg --sync-from-config --exec
```

**方式 C:独立跑(不发微信,只同步)**
```bash
python ../bundled-skills/skill-wechat-publisher/scripts/multi_publish.py --input x.md --platforms zhihu,juejin --exec
```
