...
```bash
# 手写 card.html（html,body 尺寸 = 画布尺寸，overflow:hidden），然后从 V3 runtime/ 运行：
python skills/shared/scripts/render_card.py --html card.html --out out.png \
  --width 1080 --height 1350 --scale 2
# → 2160×2700 的 2× PNG，再用 image_handler.py upload
```

`render_card.py` 使用 V3 已安装的 Playwright Chromium，macOS 与 Windows 命令一致，不依赖系统 Chrome 的固定路径。
