...
6. `python scripts/wechat_api.py list-image-styles` 验证能读出
...
```bash
# 列出所有已注册风格
python scripts/wechat_api.py list-image-styles

# 看某个风格的完整 JSON
cat assets/image-styles/marker-lime.json | python -m json.tool

# 单独跑拆卡 + 出图计划(不实际生图)
python scripts/newspic_build.py brief.md --dry-run
```
