...
典型流程:
    ...
    # 2. 拆卡 + 生成 prompts + 短文本
    python newspic_build.py brief.md
    # 3. Claude 按 card_plan.json 逐张生图,保存到 images/01.png 起
    # 4. Claude 跑 AI 味检测(newspic 模式)
    python ai_score.py article.md --mode newspic
    # 5. 发布
    python publish.py --account main --type newspic --brief brief.md
...
def main():
    ...
    print("  3. python ai_score.py <brief.md 里的短文本> --mode newspic --threshold 45")
    print(f"  4. python publish.py --account <main|tech> --type newspic --brief {brief_path}")
