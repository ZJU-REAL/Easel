...
def _bun_executable() -> str:
    ...
    bin_dir = root / "runtime" / "tools" / "node_modules" / ".bin"
    for name in (("bun.exe", "bun.cmd") if os.name == "nt" else ("bun",)):
        candidate = bin_dir / name
        if candidate.is_file():
            return str(candidate)
    raise SystemExit("V3 内置 Bun 运行时不可用，请重新运行 Seal V3 安装脚本。")
...
def main(argv: List[str]) -> int:
    ...
    env["SEAL_PYTHON"] = sys.executable
