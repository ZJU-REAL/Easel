...
python ../bundled-skills/skill-wechat-publisher/scripts/wechat_api.py list-accounts
# 验证 API 连接
cd ../bundled-skills/skill-wechat-publisher/scripts && python -c "from wechat_api import get_access_token; print('OK:', get_access_token()[:10])"
...
### 阶段五：格式转换与排版
```bash
python ../bundled-skills/skill-wechat-publisher/scripts/html_converter.py article.md --theme refined-blue -o article.html
python ../bundled-skills/skill-wechat-publisher/scripts/html_converter.py article.md --list-themes
```
...
```bash
python ../bundled-skills/skill-wechat-publisher/scripts/ai_score.py outputs/主题名/article.md --threshold 45
```
命中时按脚本列出的 AI 套话/高频词逐句**重写整个句式**（不只换词），重跑到通过。可选双保险：朱雀 / GPTZero 第三方检测。

### 阶段六：发布到草稿箱（不自动群发）
```bash
python ../bundled-skills/skill-wechat-publisher/scripts/publish.py --account main \
