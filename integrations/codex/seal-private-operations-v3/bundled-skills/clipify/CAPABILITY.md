...
- **Whisper:** 使用 V3 内置 `skills/shared/scripts/asr.py`（faster-whisper），无需另装 `whisper` CLI。英文快速扫描使用 `--model tiny --language en`；其他语言使用 `--model base --language auto`。
...
- **Scripts:** `../bundled-skills/clipify/scripts/`（从 V3 `runtime/` 运行）
...
Working dir: `outputs/_scratch/clipify/`（跨平台且仅位于 V3 内；任务完成后可清理）。
...
```bash
python -c "from pathlib import Path; Path('outputs/_scratch/clipify').mkdir(parents=True, exist_ok=True)"
ffmpeg -y -i <VIDEO> -vn -ac 1 -ar 16000 outputs/_scratch/clipify/audio.wav
python skills/shared/scripts/asr.py transcribe -i outputs/_scratch/clipify/audio.wav \
  --model tiny --language en --format json --word-timestamps \
  -o outputs/_scratch/clipify/audio.json
```
...
```bash
ffmpeg -y -ss <START> -t <DURATION> -i <VIDEO> -c copy outputs/_scratch/clipify/clip_<N>.mp4
```
...
1. **Locate the two face ROIs.** Sample one frame: `ffmpeg -ss <middle> -i <clip> -frames:v 1 outputs/_scratch/clipify/probe.jpg`. Read it. Eyeball each face's mouth+chin area as `x,y,w,h` in the source's pixel space. (No cv2 needed — camera is static within a clip; one frame is enough.) Verify by drawing boxes:
...
2. **Extract per-frame motion energy in each ROI:**
   ...
   [0:v]split=2[a][b];
   [a]crop=<LW>:<LH>:<LX>:<LY>,format=gray,tblend=all_mode=difference,signalstats,metadata=mode=print:key=lavfi.signalstats.YAVG:file=outputs/_scratch/clipify/L.txt[la];
   [b]crop=<RW>:<RH>:<RX>:<RY>,format=gray,tblend=all_mode=difference,signalstats,metadata=mode=print:key=lavfi.signalstats.YAVG:file=outputs/_scratch/clipify/R.txt[ra]
...
3. **Build speaker timeline** (min dwell 1.0s — short interjections merge into the prior speaker):
   ...
   ```bash
   python ../bundled-skills/clipify/scripts/analyze.py outputs/_scratch/clipify/L.txt outputs/_scratch/clipify/R.txt 1.0 > outputs/_scratch/clipify/segments.json
   ```
...
5. **Generate the hard-cut x expression and render:**
   ...
   ```bash
   python ../bundled-skills/clipify/scripts/build_pan.py \
     outputs/_scratch/clipify/segments.json <LEFT_X> <RIGHT_X> \
     --render-input outputs/_scratch/clipify/clip_<N>.mp4 \
     --output outputs/_scratch/clipify/clip_panned.mp4
   ```
...
python skills/shared/scripts/asr.py transcribe -i outputs/_scratch/clipify/clip_panned.mp4 \
  --model tiny --language en --format json --word-timestamps \
  -o outputs/_scratch/clipify/clip_panned.json
python ../bundled-skills/clipify/scripts/build_ass.py outputs/_scratch/clipify/clip_panned.json outputs/_scratch/clipify/captions.ass opus
```

Burn captions:

```bash
ffmpeg -y -i outputs/_scratch/clipify/clip_panned.mp4 -vf "subtitles=outputs/_scratch/clipify/captions.ass" \
  -c:v libx264 -preset fast -crf 20 -c:a copy "$OUTPUT.mp4"
```

### Step 6 — Deliver

- Save each output to `outputs/<人类可读主题>/`，不要写到源素材目录
- Print one line per clip: name, duration, what was funny, output path
- 在 Seal 工作台中返回第一个输出路径供用户检查
