...
4. 统一入口（默认 `--root templates`）：
   ```bash
   S=../bundled-skills/template-library/scripts/templates.py
   python $S save 种草对比 --category xiaohongshu --description "双产品对比" \
           --text "标题：{{title}} A={{product_a}} vs B={{product_b}} CTA={{cta}}"
   python $S list --category xiaohongshu                  # 按分类列出，按使用次数降序
   python $S get 种草对比                                  # 预览内容 + 元信息
   python $S use 种草对比 --var title=夏日防晒 --var product_a=安耐晒
   python $S use 种草对比 --vars '{"title":"夏日防晒","cta":"点赞收藏"}'
   python $S edit 种草对比 --category general --description "改描述" --text "新正文 {{x}}"
   python $S delete 种草对比                                # 预览（dry-run）
   python $S delete 种草对比 --apply                        # 确认后真正删除
   ```
