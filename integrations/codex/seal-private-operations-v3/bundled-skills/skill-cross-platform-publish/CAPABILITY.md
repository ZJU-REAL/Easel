...
脚本路径：`../bundled-skills/skill-cross-platform-publish/scripts/publish_dispatch.py`。

```bash
# 1) 生成派发计划（校验各平台越限 + 路由到 publisher）
python ../bundled-skills/skill-cross-platform-publish/scripts/publish_dispatch.py plan --manifest content.json
```
2. **按计划为每个平台适配内容**（你来做）：
