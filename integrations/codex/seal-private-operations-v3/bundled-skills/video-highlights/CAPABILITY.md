...
python skills/shared/scripts/highlight_cut.py energy -i <长视频> \
  --top 5 --clip-len 20 -o outputs/_scratch/hl_cand.json
# 2) 切片（可同时转竖版）
python skills/shared/scripts/highlight_cut.py cut -i <长视频> \
  --segments outputs/_scratch/hl_cand.json -o outputs/video-highlights \
...
1. 先转录（复用 auto-subtitle 的 `asr.py`，带时间轴）：
   ```bash
   python skills/shared/scripts/asr.py transcribe -i <长视频> --format json -o outputs/_scratch/hl.json
   ```
2. **你**读转录，挑出 3-5 个最有价值/最抓人的片段（完整语义段，别切半句），
   写成切片清单 `outputs/_scratch/hl_segs.json`：
   ```json
...
3. 切片：
   ```bash
   python skills/shared/scripts/highlight_cut.py cut -i <长视频> \
     --segments outputs/_scratch/hl_segs.json -o outputs/video-highlights --reframe 9:16
   ```
