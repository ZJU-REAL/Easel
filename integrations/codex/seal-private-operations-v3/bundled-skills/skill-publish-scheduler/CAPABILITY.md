...
脚本路径（相对项目根）：`../bundled-skills/skill-publish-scheduler/scripts/publish_queue.py`（各子命令 `-h`）。
...
4. 需要到点触发时，在 Codex App 使用原生自动化周期性运行 `run`。CLI 无自动化工具时交回 App；本脚本仅计算到期项，不常驻。
...
5. 定时触发依赖 Codex App 自动化或手动运行；必须先取得明确发布授权，并配置账号登录态。排期记录不等于自动化已启用。
