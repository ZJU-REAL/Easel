...
python ../bundled-skills/skill-publish-log/scripts/log.py record --platform 小红书 --title "标题" \
...
python ../bundled-skills/skill-publish-log/scripts/log.py query --platform 小红书 --since 2026-07-01 \
  [--until 2026-07-31] [--keyword AI] [--profile 画像名] [--latest] [--limit N]

# 统计（按 platform/type/month 聚合数量 + 互动总量 + 平均综合分）
python ../bundled-skills/skill-publish-log/scripts/log.py stat --by platform --since 2026-07-01
```

4. **解读结果**：把 JSON 转成 CAPABILITY.md「输出」节的表格/摘要格式呈现给用户；
