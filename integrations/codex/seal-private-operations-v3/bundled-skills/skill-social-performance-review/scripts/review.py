...
输入 JSON（--input）结构见 CAPABILITY.md「确定性计算」节。缺字段自动降级，
...
sys.path.insert(0, str(Path(__file__).resolve().parents[3] / "runtime" / "skills" / "shared" / "scripts"))
import social_stats as ss  # noqa: E402

# 内部评分维度权重（与 CAPABILITY.md / analysis-framework.md 一致）
SCORE_WEIGHTS = {
