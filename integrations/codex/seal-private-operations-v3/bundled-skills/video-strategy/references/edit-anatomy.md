...
Don't use it to copy someone's actual creative — this extracts the *editing grammar* (structure, rhythm, caption treatment), not the script, footage, or brand. Same rule as mining organic content for vocabulary: take the technique, never the creative.
...
- **Transcript + frames** — get the transcript *and* frames sampled at the cut points, so you can read on-screen text, caption style, and shot changes. In Easel: `video-to-article`/`clipify` (ASR via `asr.py`) for the transcript, plus `skills/shared/scripts/video_ops.py frame` to grab stills at timestamps. This is the primary input.
- **WebFetch** — pull the post for the caption, engagement, and the media URL when the reference is a specific tweet/Reel/TikTok.
...
Show the beat sheet before anyone edits anything — a review-once gate before committing footage to the edit. The reviewer checks two things:
...
- **Remotion / Hyperframes** — when you want the edit templated and data-driven (see the programmatic-video section in CAPABILITY.md); the beat sheet *is* the composition spec.
...
- **Describing instead of reading** — you can't extract caption style or cut timing from the transcript alone; pull the frames (`video_ops.py frame`, or the user's screenshots).
