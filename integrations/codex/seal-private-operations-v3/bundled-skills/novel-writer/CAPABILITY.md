...
脚本路径（相对项目根）：`../bundled-skills/novel-writer/scripts/novel_ops.py`。
...
1. **搭骨架**：`python ../bundled-skills/novel-writer/scripts/novel_ops.py scaffold --book "<书名>"`
...
8. **字数门禁**：把正文喂
   `python skills/shared/scripts/wordcount.py check --target <平台目标字数> --tolerance 0.15 -f chapters/<NNN>.md`
   （知乎盐选/番茄单章常 2000–4000，见 platform-specs），不达标按提示增删。
9. **去 AI 味**：先跑 `python ../bundled-skills/novel-writer/scripts/novel_ops.py slopcheck -f chapters/<NNN>.md`
