...
1. **写脚本分镜**：用 [video-script](../video-script/CAPABILITY.md) 把主题写成口播文案，拆成 N 句（每句一个分镜），每句配一个画面描述。

2. **生成画面**（每个分镜一张图/一段片）：
   - 有图像 API key → [ai-image-gen](../ai-image-gen/CAPABILITY.md) 逐句 text2img（按已确认画幅）
   - 要动态 → [ai-video-gen](../ai-video-gen/CAPABILITY.md) text2video/image2video
   - 用户自带素材 → 用 [image-editing](../image-editing/CAPABILITY.md) `pad` 到已确认画幅
   - 都没有 → 按已确认画幅选图卡（竖版用 card-xiaohongshu/poster-hero，横版用 card-quote）再 pad，避免画幅错配。

3. **配音**：[tts-voiceover](../tts-voiceover/CAPABILITY.md) 把文案合成口播（同时出 SRT）。**配了 `VOICE_PROVIDER` 默认走闭源好嗓子**（有情感、像真人），没 key 才退 edge（机械）——想要口播不"生硬"务必配闭源 key。不需要配音才跳过。

4. **字幕**：用 TTS 附带的 SRT，或对配音跑 [auto-subtitle](../auto-subtitle/CAPABILITY.md)；也可让 assemble 用各分镜 caption 自动生成。

5. **BGM**：[ai-music](../ai-music/CAPABILITY.md) 生成，或用用户提供的音乐。可选。
