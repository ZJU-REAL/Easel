...
SHARED_SCRIPTS = SKILL_ROOT / "runtime" / "skills" / "shared" / "scripts"
...
def cmd_generate(root: str, series: str, episode: int, only: list[int] | None,
    ...
    ai_video_script = SHARED_SCRIPTS / "ai_video.py"
...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # 复核 C01 后 → dry-run 通过
        ref_review(td, "测试剧", "C01", "看到：约三十岁的冷峻短发男性，穿深色西装，神情克制，符合男主设定")
        try:
