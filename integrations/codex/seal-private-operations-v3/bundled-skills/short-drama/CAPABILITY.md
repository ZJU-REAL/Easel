...
# AI 短剧制作（横屏/竖屏微短剧，多集）
> **配置检查路径铁律**：整条流水线开始前先 `cd` 到 `AGENTS.md` 所在的 V3 运行根，确认 `.env` 与 `skills/shared/scripts/` 存在。生图、视频、配音配置必须从这里运行注册表/`check`；不得在其他目录使用 `./shared/scripts/...`，不得用 `env` / `printenv` 判定缺少 `IMG_BASE_URL`、`VOICE_BASE_URL` 或 Key。发现缺项先核对 `pwd`，回运行根并显式传 `--env-file .env` 重查。
## ⛔ 三条铁律（最容易翻车，动手前先记死）
...
脚本（相对项目根）：`../bundled-skills/short-drama/scripts/drama_ops.py`（资产/分镜/进度）、
`../bundled-skills/short-drama/scripts/dubbing.py`（多角色配音）；
合成器复用：`../bundled-skills/auto-short-video/scripts/assemble.py`。
...
2. **搭骨架**：`python ../bundled-skills/short-drama/scripts/drama_ops.py scaffold --series "<剧名>"`。
...
9. 逐个登记进索引（自动分配 C/S/P 编号）：
   ```bash
   python ../bundled-skills/short-drama/scripts/drama_ops.py ref add --series "<剧名>" \
...
9.5 **⛔ 定妆图必须肉眼复核（做完图绝不跳过）**：用图片工具**逐张看每个角色定妆图**，确认发型/年龄/服装/气质**符合角色设定**，逐个记录：
   ```bash
   python ../bundled-skills/short-drama/scripts/drama_ops.py ref review --series "<剧名>" \
...
10. 读 `skills/shared/references/voice-casting.md`。**音色由角色的真实定妆参考图形象决定**——先看 §2 生成出来的定妆图长什么样，按 voice-casting.md〇.6「形象原型 → 音色速查」对号入座：图里是**萝莉就配萝莉音（女童声/高而快）**、御姐配成熟音、大叔配低沉老成音、霸总配浑厚音…**绝不能形象与音色错位**（女主配大妈音、小孩配成年音 = 出戏）。**每个角色用 `--archetype` 写形象原型、`--ref` 绑定其定妆图 C 编号**（`cast check` 会核对：ref_index 里每个有定妆图的角色都必须被配音，漏配直接拦）。旁白单列且与所有角色不同。
   ...
   ```bash
   python ../bundled-skills/short-drama/scripts/dubbing.py cast init --series "<剧名>"   # 建模板（旁白默认闭源）
   # 按角色定妆图形象挑贴合的闭源预置音色，并标注 archetype/ref
   python ../bundled-skills/short-drama/scripts/dubbing.py cast add --series "剧名" --name 林策 --role male_lead \
   ...
   python ../bundled-skills/short-drama/scripts/dubbing.py cast add --series "剧名" --name 朵朵 --role child \
   ...
   python ../bundled-skills/short-drama/scripts/dubbing.py cast check --series "剧名"
   ```
...
13. **规划每镜时长 + 检查能否塞进片段**（治「画面停住/太赶」的根本）：
   ```bash
   python ../bundled-skills/short-drama/scripts/dubbing.py plan --series "<剧名>" --episode N
   ```
...
16. **生成音频契约和 prompt（把台词喂给模型）**：先跑 `model_registry.py configured --group video --env-file .env`；多个可用且用户没点名时先询问，本集选定后全链路锁定。
    ```bash
    python ../bundled-skills/short-drama/scripts/dubbing.py prepare --series "<剧名>" --episode N --language zh-CN --provider "$VIDEO_PROVIDER"
    ```
...
17. **逐镜生视频（脚本驱动，强制把台词喂给模型）**：
    ```bash
    python ../bundled-skills/short-drama/scripts/drama_ops.py generate --series "<剧名>" --episode N --ratio "<9:16或16:9>"
    ```
...
18. **审计每个生成片段的原生音轨**（环境音优先，决策见 `references/native-audio-workflow.md`）：
    ```bash
    python ../bundled-skills/short-drama/scripts/dubbing.py audit --series "<剧名>" --episode N
    ```
...
20. **时间线对齐配音**（必须在每镜生成并审计后运行）：
    ```bash
    python ../bundled-skills/short-drama/scripts/dubbing.py align --series "<剧名>" --episode N
    ```
...
22. 把本集分镜转成合成输入并合成。`storyboard` 自动读取审计结果：`native` 镜**整轨原音（模型原声+环境音）直通**；`dub` 镜原生轨**丢弃、改用独立 TTS 配音**（无人声分离时保留会与配音双重人声）；完全无原音的镜补等长静音；侧链闪避仅让旁白/配音干净盖在 native 镜环境音上；合成器统一 AAC 48 kHz stereo 后拼接：
    ```bash
    python ../bundled-skills/short-drama/scripts/drama_ops.py storyboard --series "<剧名>" --episode N --size "<1080x1920或1920x1080>" \
    ...
    python ../bundled-skills/auto-short-video/scripts/assemble.py assemble \
